    ...
    config = function()
      require("neo-tree").setup({
        ...
        window = {
          width = 30,
          mappings = {
              ["w"] = "noop",    -- движение вверх
              ["s"] = "noop",    -- движение вниз
              ["S"] = "noop",
              ["x"] = "open",           -- enter открывает файл/папку
              ["q"] = "close_window",      -- закрыть окно neo-tree
              ["Q"] = function(state)
                  vim.cmd("wincmd h") -- перейти на окно слева
              end,
              ["E"] = function(state)
                  vim.cmd("wincmd l") -- перейти на окно справа
              end,
